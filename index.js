...
const defaultSettings = {
    ...
    widgetPos: null, lastCheckedMessageId: null, chatLoveData: {},
};
...
function loveData() {
    ...
    // migrate: ensure per-chat rules exist
...
function injectStyles() {
    ...
    s.id = 'ls-styles';
    s.textContent = [
        '#ls-widget{position:fixed;bottom:84px;right:18px;width:64px;height:60px;cursor:grab;z-index:9998;user-select:none;touch-action:none;filter:drop-shadow(0 4px 14px rgba(255,60,100,.35));transition:filter .2s ease;}',
        ...
        '.ls-card-pos{border:1px solid rgba(80,220,120,.3);}',
        '.ls-card-neg{border:1px solid rgba(255,90,90,.3);}',
        '.ls-card-neu{border:1px solid rgba(140,110,220,.25);}',
        '.ls-heart-box{display:flex;flex-direction:column;align-items:center;gap:3px;min-width:44px;}',
        '.ls-heart-icon{font-size:20px;line-height:1;}',
        '.ls-del-btn{padding:3px 7px!important;min-width:unset!important;align-self:flex-start;opacity:.5;transition:opacity .15s;}',
        '.ls-del-btn:hover{opacity:1;}',
        '.ls-range-box{display:flex;flex-direction:column;align-items:center;gap:4px;min-width:108px;}',
        '.ls-range-box span{font-size:9px;font-weight:700;color:rgba(160,130,230,.8);line-height:1;}',
        '.ls-range-inner{display:flex;align-items:center;gap:4px;}',
...
function buildHeartSVG(score, max) {
    ...
    const fY = (95*(1-ratio)).toFixed(2), fH = (95*ratio).toFixed(2);
...
function createWidget() {
    ...
    injectStyles();
    const d = loveData(), w = document.createElement('div');
    w.id = 'ls-widget';
    w.innerHTML = buildHeartSVG(d.score, d.maxScore) + '<div id="ls-status-tip"></div>';
...
function makeDraggable(w) {
    ...
    w.addEventListener('pointerup', () => {
        ...
        } else { cfg().widgetPos={left:w.style.left,bottom:w.style.bottom}; saveSettingsDebounced(); }
    });
}
...
function settingsPanelHTML() {
    return '<div id="ls-settings-panel" class="extension-settings">'
         ...
         + '<div class="ls-row"><span style="font-size:13px;">Значение:</span><input id="ls-val" type="number" min="0" class="ls-num-input" style="width:64px;"><span style="opacity:.5;">/</span><input id="ls-max" type="number" min="1" class="ls-num-input" style="width:64px;"><button id="ls-reset-btn" class="menu_button">&#8635; Сбросить</button></div>'
         ...
         + '<div class="ls-hint">&#10084;&#65039; &#8212; насколько растёт&#160;&#160;&#160;&#160;&#128148; &#8212; насколько падает</div>'
         ...
         + '<div class="ls-row" style="margin-top:10px;"><label class="checkbox_label" for="ls-gradual"><input type="checkbox" id="ls-gradual"><span>Медленное изменение (без резких скачков)</span></label></div>'
         + '</div></div></div>';
}

function renderChanges() {
    const ct=document.getElementById('ls-changes-container'); if(!ct) return;
    const d=loveData(), ch=d.scoreChanges||[];
    let html='';
    ch.forEach((c,i) => {
        const pos=c.delta>=0;
        ...
        html+='<div class="ls-card '+cls+'" data-idx="'+i+'">'
             ...
             +'<input type="number" class="ls-delta-input ls-num-input" value="'+c.delta+'" data-idx="'+i+'" style="width:52px;font-weight:700;font-size:14px;"></div>'
             +'<textarea class="ls-change-desc ls-textarea-field" data-idx="'+i+'" rows="3" placeholder="'+ph+'">'+escHtml(c.description)+'</textarea>'
             +'<button class="ls-del-change menu_button ls-del-btn" data-idx="'+i+'" title="Удалить">&#10005;</button></div>';
    });
...
function renderInterps() {
    ...
    arr.forEach((ip,i) => {
        ...
        html+='<div class="ls-card ls-card-neu" data-idx="'+i+'" style="'+st+'">'
             ...
             +'<button class="ls-del-interp menu_button ls-del-btn" data-idx="'+i+'" title="Удалить">&#10005;</button></div>';
    });
...
function bindChangesEv() {
    ...
    // FIX: NO renderChanges() on input — prevents focus loss
...
function bindInterpEv() {
    ...
    // FIX: NO renderInterps() on description input — this was causing focus loss!
...
function updatePromptInjection() {
    try {
        ...
        const p=buildPrompt();
        setTimeout(()=>{setExtensionPrompt(PROMPT_KEY,p,extension_prompt_types.IN_CHAT,0);console.log('[LoveScore] prompt ok');},50);
    } catch(e){console.error('[LoveScore]',e);}
}
...
jQuery(async () => {
    ...
    // FIX: per-chat rules — reset lastCheckedMessageId and reload UI (same as pregnancy extension)
    if (event_types.CHAT_CHANGED) eventSource.on(event_types.CHAT_CHANGED, ()=>{
        console.log('[LoveScore] CHAT_CHANGED - switching chat');
        cfg().lastCheckedMessageId=null;
        syncUI(); updatePromptInjection();
    });
    console.log('[LoveScore] v6 ready');
});