...
function toast(type, msg) {
  try {
    if (typeof toastr !== 'undefined') toastr[type]?.(msg, 'Love Score', { timeOut: 2300, positionClass: 'toast-top-center' });
  } catch {}
}
...
// FIX: escHtml was broken in your v9 file (it caused a JS parse error => extension doesn't load)
function escHtml(s) {
  return String(s ?? '')
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
...
/* Rescue button: always visible like fetish-manager mini button */
#ls-rescue{
  position:fixed;
  top:100px;
  right:20px;
...
// FIX: buildHeartSVG was returning an empty string in your v9 file => widget invisible
...
function applyWidgetSize(sz) {
  ...
  // restore saved right/bottom if present
...
function init() {
  ...
  // migrate: if old format used left/top, drop it
  ...
  console.log('[LoveScore] v9.1 ready');
