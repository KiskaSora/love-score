...
const defaultSettings = { isEnabled:true, maxScore:100, gradualProgression:true, widgetPos:null, lastCheckedMessageId:null, chatLoveData:{} };
const mkLoveData = () => ({
...
function loveData(){ const c=cfg(); if(!c.chatLoveData)c.chatLoveData={}; const id=getChatId(); if(!c.chatLoveData[id])c.chatLoveData[id]=mkLoveData(); const d=c.chatLoveData[id]; if(!d.scoreChanges)d.scoreChanges=mkLoveData().scoreChanges; if(!d.scaleInterpretations)d.scaleInterpretations=mkLoveData().scaleInterpretations; return d; }
...
function injectStyles(){
  ...
  s.textContent=[
    ...
    '#ls-widget:hover{filter:drop-shadow(0 6px 22px rgba(255,60,100,.6));}#ls-widget:active{cursor:grabbing;}',
...
/* ── clampWidget: keeps the heart on-screen on any device/orientation ── */
function clampWidget(){
  ...
  let bot   = parseFloat(w.style.bottom);
  // widget may still be using CSS right/bottom (never dragged) — convert
  if(!w.style.left||isNaN(left))   left = vW - wW - (parseFloat(w.style.right)||18);
  if(!w.style.bottom||isNaN(bot))  bot  = 84;
  left = Math.max(8, Math.min(vW-wW-8, left));
  bot  = Math.max(8, Math.min(vH-wH-8, bot));
  w.style.left=''+left+'px'; w.style.right='auto';
  w.style.bottom=''+bot+'px'; w.style.top='auto';
}
...
function createWidget(){
  ...
  // clamp after DOM paint — fixes off-screen on phone
...
function settingsPanelHTML(){
  return '<div id="ls-settings-panel" class="extension-settings">'
       ...
       +'<div class="inline-drawer-toggle inline-drawer-header"><b>&#10084;&#65039; Расположение</b><div class="inline-drawer-icon fa-solid fa-circle-chevron-down down"></div></div>'
...
jQuery(async()=>{
  ...
  const c=cfg();for(const[k,v]of Object.entries(defaultSettings)){if(c[k]===undefined)c[k]=structuredClone(v);}
  ...
  console.log('[LoveScore] v7 ready');
});
