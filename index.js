...
function settingsPanelHTML() {
  return '<div id="ls-settings-panel" class="extension-settings">'
    ...
    + '<div class="ls-hint">Активный диапазон влияет на поведение ИИ.</div>'
    + '<div id="ls-interp-container"></div>'
    + '<div class="ls-row" style="margin-top:10px;">'
    + '<label class="checkbox_label" for="ls-gradual"><input type="checkbox" id="ls-gradual"><span>Медленное изменение</span></label>'
    + '</div>'
    + '</div></div></div>';
}
...
function renderChanges() {
  ...
  const ch = loveData().scoreChanges || [];
  let html = '';
  ch.forEach((c, i) => {
...
function buildPrompt() {
  ...
  p += '\n\nAt end of each response include:\n<!-- [LOVE_SCORE:X] -->\nReplace X with updated integer (0-' + d.maxScore + ').';
  return p;
}
...
function bindMainEvents() {
  ...
  $(document).off('input', '#ls-size').on('input', '#ls-size', function() {
    ...
    const lb = document.getElementById('ls-size-label'); if (lb) lb.textContent = sz + 'px';
